...
if sys.platform == "linux":
    # Configuración para Ubuntu/Linux (server o local)
    if 'DISPLAY' not in os.environ:
        os.environ['QT_QPA_PLATFORM'] = 'xcb'  # Usar Xvfb
        os.environ['XAUTHORITY'] = os.path.expanduser('~/.Xauthority')  # Ruta genérica
    # Opcional: Forzar 'offscreen' si no hay GPU
    # os.environ['QT_QPA_PLATFORM'] = 'offscreen'

elif sys.platform == "win32":
    # Configuración específica para Windows
    os.environ['QT_QPA_PLATFORM'] = 'windows'  # Plugin nativo de Windows
